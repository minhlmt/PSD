...
def get_combined_attention(feature_set, param=1, exp_spatial=2, exp_channel=6, norm='l2'):
    ...
    spatial_attention_map = get_attention_new(feature_set, param=param, exp=exp_spatial, norm=norm)  # [B, H, W]
...
def train(args, epoch, generator, discriminator, optim_g, optim_d, trainloader, criterion, aug, aug_rand):
    ...
    if args.net =='convnet':
        ...
        model.load_state_dict(torch.load('/kaggle/input/svhn-model/convnet_svhn.pth', map_location=args.device))
        ...
        model.load_state_dict(torch.load('/kaggle/input/svhn-model/resnet18_svhn.pth', map_location=args.device))
